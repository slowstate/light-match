...
var teleport_shell_rotation_speed: float
...
func enter() -> void:
	...
	teleport_shell_rotation_speed = star.shell_rotation_speed * 100.0
...
func physics_update(delta: float) -> void:
	...
	if !prep_timer.is_stopped():
		...
		star.hit_box.modulate.g = lerpf(1.0, 0.0, ease(1 - prep_timer.time_left / prep_timer.wait_time, 1.0))
	if !cooldown_timer.is_stopped():
		teleport_shell_rotation_speed = lerpf(
			star.shell_rotation_speed * 100.0, star.shell_rotation_speed, ease(1 - cooldown_timer.time_left / cooldown_timer.wait_time, 0.5)
		)
	star.rotate_star(delta, teleport_shell_rotation_speed)
